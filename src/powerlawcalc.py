...
def plotting(degr_seq):
    ...
    pars, cov = curve_fit(f=power_law, xdata=degr_seq, ydata=something, p0=[0, 0], bounds=(-np.inf, np.inf))
    stdevs = np.sqrt(np.diag(cov))
    print(*pars)
    ax = fig.add_subplot()
    plt.hist(degr_seq, bins=100)
    ax.plot(degr_seq, power_law(degr_seq, *pars), linestyle='--', linewidth = 2, color='r')
    plt.show()
